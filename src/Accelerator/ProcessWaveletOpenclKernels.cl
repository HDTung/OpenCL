static const char* g_wavelet_kernel_src = R"---(

    #define WRAP(x, dx, n) ((dx < 0) ? (((x) + dx >= 0) ? ((x) + dx) : ((x) + dx + n)) \
                                 : (((x) + dx <  n) ? ((x) + dx) : ((x) + dx - n)))
    #define LOAD(x, y) *((__global const float2*)(src + mad24((x), sizeof(float2), mad24((y), src_step, src_offset))))
    #define STORE(x,y) *((__global float2*)(dst + mad24((x), sizeof(float2), mad24((y), dst_step, dst_offset))))
    #define CONJ(x) (float2)((x).s0, -(x).s1)
    #define NOT(x) (float2)(-(x).s0, -(x).s1)

    __kernel void decompose_vertical(__global const uchar *src, int src_step, int src_offset,
                                     __global uchar *dst, int dst_step, int dst_offset,
                                     int rows, int cols, float16 lopass, float16 hipass)
    {
